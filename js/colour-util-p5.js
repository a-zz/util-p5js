...
(function(context) { 
  ...
  context.paletteMono3 = function(hue, s) {
    ...
    var palette = [];
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue, s) 	 + ',  40%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue-30, s) + ',  20%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue+45, s) + ', 100%, 100%)'));
    return palette;
  }
  ...
  context.paletteMono5 = function(hue, s) {
    ...
    var palette = [];
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue, s) 	 	+ ',  40%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue-15, s) 	+ ',  40%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue+15, s) 	+ ',  40%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue-30, s) 	+ ',  20%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue+45, s) 	+ ', 100%, 100%)'));
    return palette;
  ...
  context.paletteCompl3 = function(hue, s) {
    ...
    var palette = [];
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue, s)			+ ',  40%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue+180, s) + ',  20%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue+150, s) + ', 100%, 100%)'));
    return palette;
  }
  ...
  context.paletteTrio3 = function(hue, s) {
    ...
    var palette = [];
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue, s) 		+ ',  40%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue+120, s) + ',  20%, 100%)'));
    palette.push(s.color('hsb(' + AZZP5.fixHue(hue-120, s) + ', 100%, 100%)'));
    return palette;
  }
  ...
  /* fixHue()
   * Corrects a hue value for P5 admisible values (0-359), which comes handy for
   *  hue shifting operations.
   * Args:  hue (integer) The hue to correct (if needed to).
   *        s (p5 sketch) For multi-instance operation; not needed for 
   *          single (global) operation.
   * Rtrn:  (integer) The corrected hue.
   */
  context.fixHue = function(hue, s) {

    s = s!=null?s:p5.instance;
    return s.round(hue % 360 + (hue < 0?360:0));
  }
})(AZZP5);
/* ************************************************************************** */
